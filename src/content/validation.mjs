...
const VALID_FEEDBACK_TYPES = new Set(["bug", "existing-feature-issue", "feature-suggestion"]);
...
function validateReleaseChange(c, ctx, issues) {
  ...
  if (looksLikePlaceholder(c.title)) issues.push(`${ctx}: change title looks like placeholder`);
}
...
function validateRelease(r, ctx, issues) {
  ...
  if (!ISO_DATE.test(r.releaseDate)) issues.push(`${ctx}: releaseDate must be ISO date YYYY-MM-DD`);
...
function validateFeedback(item, ctx, releaseSlugs, issues) {
  ...
  if (looksLikePlaceholder(item.category)) issues.push(`${ctx}: category looks like placeholder`);
  if (looksLikePlaceholder(item.publicDescription))
    issues.push(`${ctx}: publicDescription looks like placeholder`);
  if (typeof item.voteCount !== "number" || !Number.isInteger(item.voteCount) || item.voteCount < 0)
...
export async function getInvalidContentEntries() {
  ...
  if (!Array.isArray(releases)) issues.push("updates.ts: must export an array");
    ...
    releases.forEach((r, i) => {
      ...
      if (r && r.slug) {
        if (seenSlugs.has(r.slug)) issues.push(`releases[${i}]: duplicate slug "${r.slug}"`);
        seenSlugs.add(r.slug);
      }
    });
  }

  const releaseSlugs = new Set(Array.isArray(releases) ? releases.map((r) => r.slug) : []);
  if (!Array.isArray(publicFeedback)) issues.push("public-feedback.ts: must export an array");
  else {
    const seenSlugs = new Set();
